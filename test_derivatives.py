...
print(f"  f''({x_test}) = {second_derivatives[0](x_test)(x_test):.6f} (expected: 0.0)")
print(f"  f'''({x_test}) = {third_derivatives[0](x_test)(x_test):.6f} (expected: 0.0)")

print(f"\nf(x) = x²:")
print(f"  f({x_test}) = {quadratic_func(x_test)}")
print(f"  f'({x_test}) = {derivatives[1](x_test):.6f} (expected: {2*x_test})")
print(f"  f''({x_test}) = {second_derivatives[1](x_test)(x_test):.6f} (expected: 2.0)")
print(f"  f'''({x_test}) = {third_derivatives[1](x_test)(x_test):.6f} (expected: 0.0)")

print(f"\nf(x) = x³:")
print(f"  f({x_test}) = {cubic_func(x_test)}")
print(f"  f'({x_test}) = {derivatives[2](x_test):.6f} (expected: {3*x_test**2})")
print(f"  f''({x_test}) = {second_derivatives[2](x_test)(x_test):.6f} (expected: {6*x_test})")
print(f"  f'''({x_test}) = {third_derivatives[2](x_test)(x_test):.6f} (expected: 6.0)")

print(f"\nf(x) = sin(x):")
print(f"  f({x_test}) = {sine_func(x_test):.6f}")
print(f"  f'({x_test}) = {derivatives[3](x_test):.6f} (expected: {numpy.cos(x_test):.6f})")
print(f"  f''({x_test}) = {second_derivatives[3](x_test)(x_test):.6f} (expected: {-numpy.sin(x_test):.6f})")
print(f"  f'''({x_test}) = {third_derivatives[3](x_test)(x_test):.6f} (expected: {-numpy.cos(x_test):.6f})")